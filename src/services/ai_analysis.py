...
async def ai_analysis(property_details: str, compose_email_prompt: str, number_of_domains:int = 10):
    ...
    await set_redis_value("- Trying to get Company domains from provided property_details ...")
    ...
    await set_redis_value("- Starting loop on them ...")
    for i, company_domain in enumerate(company_domains):
        ...
        await set_redis_value(f"--- Extraction results : {str(emails)}")
        await set_redis_value("--- Starting loop on them ...")
    ...
    await set_redis_value("--------------- Successfully ended analysis ---------------")